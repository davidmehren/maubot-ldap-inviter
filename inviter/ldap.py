...
async def get_all_matrix_users_of_sync_room(
    config, evt: MessageEvent, con: SimpleLDAPObject, sync_room: SyncRoomConfig
) -> UserInfoMap:
    user_info_map = {}
    for ldap_config in sync_room["ldap_members"]:
        user_info_map.update(
            await get_matrix_users_of_ldap_group(
                config, evt, con, ldap_config["ldap_group"], ldap_config["power_level"]
            )
        )
